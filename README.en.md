...
A Tampermonkey userscript that reads chapter content through per-site adapters and translates it with the unofficial Google Translate endpoint or any OpenAI-compatible LLM API. Translations are cached per chapter path in Tampermonkey storage, so re-opening a chapter loads instantly from cache.
...
- Per-chapter-path cache; a successful segment retry updates the cache automatically.
...
- Toggle translations on/off anytime (button or `Alt+H`) to compare with the original text.
...
When enabled, after a chapter is translated the script automatically jumps to the next chapter and keeps translating until the last chapter or until you turn the option off. Currently supported on Qidian chapter pages only; the checkbox is greyed out on Fanqie and Jinjiang pages.
...
> Placeholder — add screenshots of the floating toolbar, settings panel, and translation output on a supported site.
